...
<sql:query var="searchBooks" dataSource="jdbc/libmanage">
    ...
    FROM (book inner join book_copy on book.ISBN=book_copy.ISBN) inner join authors on book.ISBN=authors.ISBN where book_copy.is_issued = 0 and is_damaged = 0 and authors = ?<sql:param value="${param.author}"/> group by book.isbn
...
<html>
    ...
    <center>
        ...
        <br>
        <form name="seatchBook" method="get" action ="selectBook.jsp">
        <select name="search">
            <c:forEach var="row" items="${searchBooks.rowsByIndex}">
